...
<body class="container mt-5">
    <div class="d-flex justify-content-between mb-3">
        ...
        <div>
            <a href="{{ route('karyawan.pdf') }}" class="btn btn-danger">Cetak PDF</a>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $k->Nama }}</td>
                <td>{{ $k->{'Tgl Lahir'} }}</td>
                <td>Rp {{ number_format($k->Gaji, 2) }}</td>
                <td>
                    <button
                        onclick="editKaryawan({{ $k->Id }}, '{{ $k->Nama }}', '{{ $k->{'Tgl Lahir'} }}', {{ $k->Gaji }})"
                        class="btn btn-warning btn-sm">Edit</button>
                    <button onclick="hapusKaryawan({{ $k->Id }})" class="btn btn-danger btn-sm">Hapus</button>
                </td>
            </tr>
            @endforeach
        </tbody>
